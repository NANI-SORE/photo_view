...
class PhotoViewScaleGestureRecognizer extends ScaleGestureRecognizer {
  ...
  void handleEvent(PointerEvent event) {
    ...
    if (event is PointerMoveEvent) {
      if (_areMultiPointers()) {
        ...
        final isPriorityMove = _canPanX() || _canPanY();
  ...
  void resolve(GestureDisposition disposition) {
    switch (disposition) {
      case GestureDisposition.accepted:
        ...
        final canAccept = _areMultiPointers() ||
            _isPriorityGesture() ||
            _canPanX() ||
            _canPanY();
        super.resolve(canAccept
  ...
  bool _isPriorityGesture() {
    ...
    if (scope.acceptPointerEvent?.call(move) ?? false) {
      return true;
    }

    // e.g. double tap & drag for one finger zoom
    if (doubleTapDetails.value != null) {
      return true;
    }
  ...
  bool _canPanX() =>
      hitDetector != null && hitDetector!.shouldMoveX(move) && isXPan(move);

  bool _canPanY() =>
      hitDetector != null && hitDetector!.shouldMoveY(move) && isYPan(move);
